...
export default function NavSidebar({ user: initialUser }: { user: NavUser }) {
  ...
  return (
    <>
      ...
      <nav className="fixed bottom-0 left-0 right-0 z-50 flex md:hidden items-center justify-around border-t border-neutral-200 bg-white px-2 py-2 safe-area-pb">
        {navItems.map(({ href, icon: Icon, label }) => (
          <Link
            key={href}
            href={href}
