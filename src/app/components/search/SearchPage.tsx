...
export default function SearchPage() {
  ...
  const handleChange = (value: string) => {
    setQuery(value);
    // Clear any pending debounce before setting a new one
  ...
  return (
    <div>
      {/* Sticky search header — matches other page headers */}
            ...
            {results.map((user) => (
              <li key={user.id}>
                <Link
                  ...
                  className="flex items-center gap-3 py-3 hover:bg-neutral-50 dark:hover:bg-neutral-800/60 rounded-xl px-2 -mx-2 transition-colors"
                >
                  ...
                  <div className="min-w-0 flex-1">
                    <p className="truncate text-sm font-semibold text-neutral-900 dark:text-neutral-100">
                      ...
                      {user.isPrivate && (
                        <span className="ml-1.5 text-xs font-normal text-neutral-400 dark:text-neutral-500">Private</span>
                      )}
                    </p>
